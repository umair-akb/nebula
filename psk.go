...
func NewPskFromConfig(c *Config, myVpnIp uint32) (*Psk, error) {
...
func NewPsk(mode PskMode, keys []string, myVpnIp uint32) (*Psk, error) {
	psk := &Psk{
		mode: mode,
	}
...
func (p *Psk) MakeFor(vpnIp uint32) ([]byte, error) {
	...
	_, err := io.ReadFull(hkdf.Expand(sha256.New, p.key, int2ip(vpnIp)), hmacKey)
	if err != nil {
		return nil, err
	}
...
func (p *Psk) cachePsks(myVpnIp uint32, keys []string) error {
...
func sha256KdfFromString(secret string, vpnIp uint32) ([]byte, error) {
	if len(secret) < MinPskLength {
		return nil, ErrKeyTooShort
	}

	hmacKey := make([]byte, sha256.Size)
	_, err := io.ReadFull(hkdf.New(sha256.New, []byte(secret), nil, int2ip(vpnIp)), hmacKey)
	if err != nil {
		return nil, err
	}
	return hmacKey, nil
